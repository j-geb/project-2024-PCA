...
The client also provided example papers to guide methodological replication. *Xu et al* (2019) extensively employs hierarchical clustering and dendrogram visualisations, which we incorporate later in the report. Additionally, the paper includes meta-analytical techniques, which may be applicable should the client obtain relevant qualitative data. Meanwhile, *Hsieh & Fischer* (2019) and *Mitchell et al.* (2012) primarily illustrate the nature of the data to be handled, along with specific analytical methods relevant to this context.
...
p1
```
**Figure 1. Distribution of each element under standardisation. The outliers (the dots on the plot) are moderate, but PCA can handle this fine. Note the shorter boxes (e.g. `pb` or `mo`) are expected to be less informative.**
...
p2
```
**Figure 2. Correlation heatmap showing the strength and direction of linear relationships between elemental concentrations. See for example, `la` and `zr` exhibit strong positive correlation (r=0.866)  indicating that they will contribute similarly in the PCA.**
...
**Figure 5. Loading vectors in 2D. Earlier assumptions on closeness of certain elements (e.g. `la` and `zr`) hold true (highly correlated).**
...
#Plot dendrogram
plot(hc1, labels = medians$bricc_standard_name, main = "Dendrogram for Scaled Medians",  xlab = "", sub="")
```
...
plot(hc2, labels = medians$bricc_standard_name, main = "Dendrogram for PCA Scores", xlab = "", sub="")
```
**Figure 9. Hierarchical clustering tree post-PCA. Clearly, minimal changes occur.**

Note: this uses the `ward.D2` method to find clusters. This minimises total within-cluster variance (recommended and presented in papers provided; explicitly from D. Mitchell et al (2012) and W. Xu et al., (2019)), however, others do exist for consideration:

- `average`: average linkage.
- `complete`: Max distance between clusters.
- `single`: Min distance between clusters.
- If the client wishes to learn more, visit [*Agglomerative Hierarchical Clustering*](https://www.datanovia.com/en/lessons/agglomerative-hierarchical-clustering/#google_vignette)

If we want a more visual application of these clusters, we shift focus onto the PCA space of the two principal components. A silhouette analysis is used to find the optimal number of components:

```{r, warning=FALSE}
p10 <- fviz_nbclust(scores, FUN = hcut, method = "silhouette", k.max = 10)   #from factoextra package
p10
```
**Figure 10. Silhouette analysis to find optimal cluster number. This method yields an optimal value of 3 clusters. The technique evaluates the quality of clusters using the silhouette width; essentially a ratio of how similar an observation is to others in its cluster compared to other clusters. The maximum silhouette width provides the optimal cluster number (closer to 1 is better).**
...
- If the client wishes to learn more, visit [*Determining The Optimal Number Of Clusters: 3 Must Know Methods*](https://www.datanovia.com/en/lessons/determining-the-optimal-number-of-clusters-3-must-know-methods/)
...
df2 <- as.data.frame(results$x[, 1:2])  #new dataframe to assign clusters
df2$cluster <- factor(clusters)
df2$label <- medians$bricc_standard_name
df2$hover_text <- paste("Brick:", df$label, "<br>Cluster:", df$cluster)

#Create plot
p14 <- ggplot(df2, aes(x = PC1, y = PC2, color = cluster, fill = cluster)) +
  stat_ellipse(geom = "polygon", type = "norm", alpha = 0.2,aes(group = cluster), show.legend = FALSE) +
  geom_point(aes(text = hover_text), size = 1.5) +  # only here
  geom_text(aes(label = label), vjust = -0.8, size = 3, show.legend = FALSE) +
  scale_color_brewer(palette = "Set1") +
...
#Shared components: loadings as segments + labels
loadings_layer <- list(
  geom_segment(data = loadings_s, 
               aes(x = 0, y = 0, xend = PC1, yend = PC2),
               arrow = arrow(length = unit(0.2, "cm")),
               color = "dodgerblue", inherit.aes = FALSE),
  geom_text(data = loadings_s,
            aes(x = PC1, y = PC2, label = element),
            color = "dodgerblue", vjust = -0.5, size = 3, inherit.aes = FALSE))

#Display plot
p_hc <- ggplot(df2, aes(x = PC1, y = PC2, color = cluster, fill = cluster)) +
  stat_ellipse(geom = "polygon", type = "norm", alpha = 0.2, aes(group = cluster), show.legend = FALSE) +
  geom_point(aes(text = hover_text), size = 1.5) +
  geom_text(aes(label = label), vjust = -0.8, size = 3, show.legend = FALSE) +
  scale_color_brewer(palette = "Set1") +
  scale_fill_brewer(palette = "Set1") +
  theme_minimal() +
  loadings_layer +
  ggtitle("Biplot with Hierarchical (left) and K-Means (right) Clustering")

p_km <- ggplot(df, aes(x = PC1, y = PC2, color = cluster, fill = cluster)) +
  stat_ellipse(geom = "polygon", type = "norm", alpha = 0.2, aes(group = cluster), show.legend = FALSE) +
  geom_point(aes(text = hover_text), size = 1.5) +
  geom_text(aes(label = label), vjust = -0.8, size = 3, show.legend = FALSE) +
  scale_color_brewer(palette = "Set1") +
  ...
  theme_minimal() +
  loadings_layer +
  ggtitle("Biplot with K-Means Clustering")

subplot(ggplotly(p_km, tooltip = "text"),
        ggplotly(p_hc, tooltip = "text"),
        nrows = 1, margin = 0.05, titleX = TRUE, titleY = TRUE)
```
**Figure 14. Biplot with clustering labels from both methods and confidence ellipses. Note once more, the clusters without ellipses may signify outliers or bricks from a different site (or area of a site), and blue is used for the scaled loadings. Comparatively, these methods are similar on this data, and the same "outlier" clusters appear in both. The hierarchical method (left) creates more clusters than k-means (right).**
